...
function Riddle() {
    ...
    firebase.firestore().collection('Registration Number Riddle Competition').doc('unique').get().then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        firebase.firestore().collection('Registration Number Riddle Competition').doc('unique').update({
            number: regNo + 1
        })

        firebase.firestore().collection('Contestant Riddle Competition').add(
            {
                "Name": name,
                "Phone no.": phone,
                "Email": email,
    ...
    return (
        <div className='riddle'>
            <div className='head-box'>
                <div className='head'>
                    <h1>RIDDLE COMPETITION</h1>
                    <p>
                        For those who are creative thinkers and loves challenges , here is something which you can participate...
                        Solve the Riddles and win for the pricepool ..!
                    </p>
                </div>
            </div>
            <div className='form'>
